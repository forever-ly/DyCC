...
# 忽略特定类型的警告
warnings.filterwarnings("ignore", message="It is not recommended to directly access the internal storage format `data`")
...
class TokenMAEClf(torch.nn.Module):
    ...
    d_model=128, trans_encoder_layer=0, nhead=4, dim_feedforward=512, transformer_dropout=0, transformer_activation=F.relu, transformer_norm_input=True, custom_trans=False, args=None):
        ...
        self.tokenizer = GNN_v2(token_layer, emb_dim, True, JK=JK, drop_ratio=drop_ratio, gnn_type=gnn_type, gnn_activation=args.gnn_activation)
        self.gnn_act = get_activation(args.gnn_activation)
        self.encoder = GNN_v2(encoder_layer-token_layer, emb_dim, False, JK=JK, drop_ratio=drop_ratio, gnn_type=gnn_type, gnn_activation=args.gnn_activation,
...
def load_dataset_chem(args):
    if args.pe_type != 'none':
        ...
        dataset = MoleculeDataset(
            "../../mae/chem/dataset/" + args.tuning_dataset, dataset=args.tuning_dataset)
    print(dataset)
    if args.split == "scaffold":
        smiles_list = pd.read_csv(
            "../../mae/chem/dataset/" + args.tuning_dataset + '/processed/smiles.csv', header=None)[0].tolist()
...
def tuning_chem(args, train_dataset, valid_dataset, test_dataset, running_seeds):
    ...
    for runseed in tqdm(running_seeds):
        ...
        print(train_roc_list,val_roc_list)
        name = args.model_file.split("/")[-2]
        sub_name = args.model_file.split("/")[-1][:-4]

        result_filename = os.path.join("./results/",name,f"{sub_name}_{args.trans_encoder_layer}_{args.tuning_dataset}_result.json")
        os.makedirs(os.path.dirname(result_filename), exist_ok=True)
        res_record = json.dumps({"seed": [args.runseed], "train":[train_roc_list[-1]],"val": [val_roc_list[-1]], "test": [test_roc_list[-1]],"train_list":train_roc_list, "val_list":val_roc_list,"test_list": test_roc_list})
        # 保存 JSON 数据到文件
        with open(result_filename, 'a') as json_file:
...
def main():
    ...
    parser.add_argument('--device', type=int, default=0,
                        help='which gpu to use if any (default: 3)')
    ...
    parser.add_argument('--model_file', type=str, default='./results/SimSGT/model_40.pth',
                        help='filename to read the model (if there is any)')
    # parser.add_argument('--model_file', type=str, default='./checkpoints/m35_ckt.pt',
    #                     help='filename to read the model (if there is any)')
    ...
    parser.add_argument('--eval_train', action='store_true',
                        help='Evaluate the training dataset or not.',default=True)
