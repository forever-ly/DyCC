...
def train_mae(args, model, loader, optimizer, epoch):
    ...
    for step, batch in enumerate(train_bar):
        ...
        if args.ib_remask:
            pred_loss,KL_loss,rate = model(batch)
            loss = pred_loss+KL_loss*0.1
        else:
            loss = model(batch)
        ...
        if args.ib_remask:
            train_bar.set_description(f"epoch:{epoch},loss:{float(loss)},pred_loss:{float(pred_loss)},KL_loss:{float(KL_loss)},rate{rate}")
        else:
...
def main():
    ...
    parser.add_argument('--num_proto', type=int, default=128)
    ...
    if args.pe_type != 'none':
        ...
        dataset = MoleculeDataset("../../mae/chem/dataset/" + dataset_name, dataset=dataset_name)
